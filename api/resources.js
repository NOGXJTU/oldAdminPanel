...
const activity = {
  data: [],
  fields: {
    [config.primaryKey]: { listable: false},
    ...
    type: {
      type: 'radiolist', cols: 3, options: [
        { text: '文化课', value: 'whk' },
        { text: '兴趣课', value: 'xqk' },
        { text: '思想沙龙', value: 'sxsl' },
        { text: '公益晚会', value: 'gywh' },
        { text: '其他', value: 'others' },
      ], searchable: true, description: '请选择活动类型'
    },
    slug: { cols: 3, searchable: true },
    name: {label: '名字', cols: 6, searchable: true, description: 'Give me an awesome title.' },
    // views: { type: 'number', cols: 3, listable: false },
    presented: { type: 'switch', cols: 3 },

    image: { type: 'image', cols: 6 },
    conclusion: { type: 'html', listable: false, cols: 6 },
    description: { type: 'html', listable: false, cols: 6 },
...
_.times(5, i => {
  ...
  activity.data.push({
    ...
    name: name,
    slug: name.slugify(),
    image: 'http://due.xjtu.edu.cn/__local/D/7B/5D/B2DEDE0BBA769F449E666FF3E94_5D1FA4D3_6287.jpg',
    // image: faker.image.imageUrl(),
    // image: faker.image.image(320, 180),
    conclusion: faker.lorem.paragraphs(),
    description: faker.lorem.paragraphs(),
    presented: Math.random() > 0.2,
    // views: parseInt(Math.random() * 1000),
    created_at: faker.date.recent()
  })
})
...
module.exports = { users, products, categories, activity, genId }