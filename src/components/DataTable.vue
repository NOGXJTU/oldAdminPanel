  ...
  <b-card :header="header">
    ...
    <div class="data-table">
      <div class="py-1">
        ...
        <b-btn v-for="button in _.get(fields || {}, '_actions.toolbar.extra', [])" :key="button.label" v-bind="button">
          {{button.label}}
        </b-btn>
...
export default {
  ...
  methods: {
    ...
    show(item) {
      this.$router.push({
        path: this.resource + "/" + item['id']
      });
    },
    edit(item) {
      this.$router.push({
        path: this.resource + "/" + item['id'] + "/edit"
      });
    },
    remove(item) {
      if (window.confirm(this.$t("messages.confirm_delete"))) {
        this.$http
          .delete(this.resourceUri + "/" + item['id'])
          .then(() => {
