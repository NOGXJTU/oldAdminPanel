          ...
          <div class="card-group">
            <div class="card p-4">
              <div class="card-body">
                ...
                <b-form-builder action="user/signIn" :fields="fields" v-model="model" 
...
export default {
  ...
  data() {
    return {
      fields: {
        passport: {
          label: this.$t("fields.passport"),
          placeholder: this.$t("fields.passport"),
          icon: "icon-user"
        },
        password: {
  ...
  methods: {
    onSuccess(data) {
      ...
      // this.$store.dispatch(types.FETCH_SITE); //不需要，本地指定
      this.$router.push({
        path: data.redirect || '/'
      });
    }
